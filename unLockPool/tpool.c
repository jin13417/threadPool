...
#define debug(level, ...) do { \
    if (level < TPOOL_DEBUG) {\
        flockfile(stdout); \
        printf("###%u.%s: ", (void *)pthread_self(), __func__); \
        printf(__VA_ARGS__); \
        putchar('\n'); \
        fflush(stdout); \
        funlockfile(stdout);\
...
#ifdef DEBUG
    printf("%u migrate_thread_work: %u\n", from->id, thread_queue_len(from));
#endif
    return 0;
}
/*正向*/
...
static void balance_thread_load(tpool_t *tpool)
{
    ...
    for (i = 0; i < tpool->num_threads; i++)
        count[i] -= avg;
    while (1) {
	/*first_neg_id 线程数量小于平均值的线程ID
	  *first_pos_id 线程数量大于平均值的线程ID*/
        first_neg_id = get_first_id(count, tpool->num_threads, isnegtive);
        first_pos_id = get_first_id(count, tpool->num_threads, ispositive);
	 printf("\r\n22222222222: %u %u ",first_neg_id,first_pos_id);
        if (first_neg_id < 0)
        	{
        	 printf("\r\n3333333: %u %u ",first_neg_id,first_pos_id);
            break;
        	}
    ...
    for (i = 1; i < count[first_pos_id]; i++) {
        ...
        if (to == from)
            continue;
	printf("\r\n,2222to :%u,form:%u,%u,%u,%u",to->id,from->id,count[first_pos_id],i,tpool->num_threads);
        work = get_work_concurrently(from);
        if (work) {
            dispatch_work2thread(tpool, to, work->routine, work->arg);
        }
